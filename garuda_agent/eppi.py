...
class EPPISensor:
    ...
    def _initialize_ebpf(self) -> None:
        """Attempt to load and attach eBPF kprobes."""
    ...
    def _handle_perf_event(self, cpu: int, data: Any, size: int) -> None:
        """Callback for perf ring buffer events."""
        if not self._bpf:
            return
    ...
    def read_events(self, timeout_ms: int = 50) -> List[Dict[str, Any]]:
        """Poll perf ring buffer and return accumulated events."""
    ...
    def inject_synthetic_event(self, event_dict: Dict[str, Any]) -> None:
        """Testing hook for synthetic event injection in test environments."""
