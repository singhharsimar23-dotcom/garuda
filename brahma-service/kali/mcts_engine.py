...
class MCTSNode:
    ...
    def get_path(self) -> List[Tuple[str, str]]:
        """Return list of (technique_id, tactic) from root to this node."""
        path = []
        curr = self
        while curr:
    ...
    def ucb1(self, c_exploration: float = 1.414) -> float:
        """Calculate Upper Confidence Bound 1 (UCB1) for child selection."""
        if self.visits == 0:
            return float("inf")
        exploitation = self.total_reward / self.visits
        parent_visits = self.parent.visits if self.parent else self.visits
        exploration = c_exploration * math.sqrt(math.log(parent_visits + 1) / self.visits)
        # Weight exploration by APT36 preference
...
class KaliMCTSEngine:
    ...
    def is_terminal(self, path: List[Tuple[str, str]]) -> bool:
        """Check if search reached maximum length or target endgame tactics."""
        if not path:
            return False
    ...
    def synthesize_novel_paths(
        ...
        # Run MCTS Iterations
        for _ in range(sims):
            # Check 30-second safeguard timeout
        ...
        # Extract all unique candidate paths explored
        ...
        # Sort paths by adversary utility descending
        ranked = sorted(
        ...
        logger.info(f"KALI ANPS complete: generated {len(ranked)} top paths ({completed_sims} simulations).")

        # Persist to Supabase if client is provided
        if supabase_client and ranked:
            try:
                for disc in ranked:
                    supabase_client.table("kali_discoveries").upsert(
                        disc, on_conflict="discovery_id"
                    ).execute()
            except Exception as e:
                logger.warning(f"Failed persisting kali_discoveries to Supabase: {e}")
